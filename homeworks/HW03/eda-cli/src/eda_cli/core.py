...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Присутствие константных столбцов
    constant_cols = [len(df[column].unique()) == 1 for column in df.columns]
    flags['has_constant_columns'] = any(constant_cols)   
    constant_score = np.round(2**(-sum(constant_cols)), 2)

    # Наличие столбцов с большим количеством уникальных значений
    high_cardinality  = [len(df[column].unique()) > 50  for column in df.columns if df[column].dtype =='O']
    flags["has_high_cardinality_categoricals"] = any(high_cardinality)
    high_cardinality_score = np.round(2**(-sum(high_cardinality)), 2)


    score = max(0.0, min(1.0, score))
    flags["quality_score"] =  (score + constant_score + high_cardinality_score) / 3
